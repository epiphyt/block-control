...
const addControls = createHigherOrderComponent( ( BlockEdit ) => {
	return ( props ) => {
		...
		const onChangeConditionalTags = ( tag, value ) => {
			...
			const newValue = hideConditionalTags
				? JSON.parse( JSON.stringify( hideConditionalTags ) )
				: {};
			newValue[ tag ] = value;
		...
		const onChangePosts = ( id, type, value ) => {
			// make sure the value gets updated correctly
			// @see https://stackoverflow.com/questions/56452438/update-a-specific-property-of-an-object-attribute-in-a-wordpress-gutenberg-block#comment99517264_56459084
			let newValue = hidePosts
				? JSON.parse( JSON.stringify( hidePosts ) )
				: {};
		...
		const onChangeHideRoles = ( role, value ) => {
			// make sure the value gets updated correctly
			// @see https://stackoverflow.com/questions/56452438/update-a-specific-property-of-an-object-attribute-in-a-wordpress-gutenberg-block#comment99517264_56459084
			let newValue = hideRoles
				? JSON.parse( JSON.stringify( hideRoles ) )
				: {};
			newValue[ role ] = value;
					...
					>
						...
						<div className="block-control-control-area">
							<RadioControl
								...
								onChange={ ( value ) =>
									setAttributes( { loginStatus: value } )
								}
							/>
						</div>
									...
									( tag, index ) => {
										return (
											<CheckboxControl
												key={ index }
												label={
													CONDITIONAL_TAGS[ tag ]
												}
												checked={
													hideConditionalTags &&
													hideConditionalTags[ tag ]
														? true
														: false
												}
												value={ tag }
												onChange={ ( value ) =>
													onChangeConditionalTags(
														tag,
														value
													)
												}
